...
- `cache-backend`: `registry`, `local`, or `native`, default `registry`.
  `native` keeps OCI import planning while delegating cache publication to the
  CLI publisher that runs inside the BuildKit builder.
