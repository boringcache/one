...
Release alignment note: the current action release candidate is `v1.12.84` and the default CLI install version is `v1.12.64`. Keep `action.yml`, `lib/utils.ts`, restore tests, generated `dist/**`, and package version changes together for future CLI default bumps, and do not treat the bump as public until it is signed, tagged, and `one@v1` is moved.
