...
interface CliCheckResult {
  ...
  kv_total_size?: number;
}

interface RustTagCheckStatus {
  hit: boolean;
...
function getModeStateBoolean(key: string): boolean {
  return getModeState(key) === 'true';
}

async function verifyOciPromotionRefsAfterStop(): Promise<void> {
  ...
  let verificationProxyPid: number | null = null;
  try {
    ...
    if (!readiness?.ready) {
      throw new Error(
        `OCI promotion refs were not readable after proxy shutdown. readable=[${readiness?.readableRefs.join(', ') || ''}] unreadable=[${readiness?.unreadableRefs.join(', ') || refs.join(', ')}]`,
      );
    }
    core.info(`Verified OCI promotion refs after proxy shutdown: ${readiness.readableRefs.join(', ')}`);
  } catch (error) {
    throw new Error(
      `OCI promotion refs were not readable after proxy shutdown. requested=[${refs.join(', ')}]: ${errorMessage(error)}`,
    );
  } finally {
...
function ociPromotionVerificationTimeoutMs(): number {
...
async function verifyOciPromotionRefsThenStopProxy(proxyPid: string): Promise<void> {
  try {
...
function recordOciRegistryPlanState(ociPlan: CliAdapterDryRunPlan, cacheTag: string): {
  resolvedWorkspace: string;
  resolvedCacheTag: string;
  registryVerification: { noPlatform: boolean; noGit: boolean; saveExpected: boolean };
  registryOciCache: CliAdapterDryRunPlan['oci_cache'];
} {
...
function dockerBuildxArgs(opts: DockerBuildOptions): string[] {
...
function ociAdapterCliArgsForAcceleratedBuild(
  adapter: 'docker' | 'buildkit',
  workspace: string,
  cacheTag: string,
...
async function buildDockerImageWithCliAdapter(
  ...
  const args = ociAdapterCliArgsForAcceleratedBuild(
    'docker',
    workspace,
    cacheTag,
    cacheBackend,
...
function buildctlArgs(opts: BuildctlOptions): string[] {
...
async function buildWithBuildctlCliAccelerator(
  workspace: string,
  cacheTag: string,
  cacheBackend: DockerCacheBackend,
  port: number,
...
function emptyRustTagCheckStatus(): RustTagCheckStatus {
  return {
    hit: false,
    cacheEntryHit: false,
    kvHit: false,
...
function checkResultHasCacheEntryHit(result: CliCheckResult): boolean {
...
async function checkRustTagStatus(
  workspace: string,
  tag: string,
  {
    noPlatform = false,
...
async function checkRustTagHit(
  workspace: string,
  tag: string,
  options: { noPlatform?: boolean; noGit?: boolean; requireServerSignature?: boolean } = {},
): Promise<boolean> {
...
async function runBuildkitRestore(plan: ResolvedPlan, inputs: OneInputs): Promise<ModeRestoreResult> {
  ...
  if (registryCachePlan) {
    ...
    if (usesCliCacheAccelerator(cacheBackend)) {
      ...
      await buildWithBuildctlCliAccelerator(
        dockerPlan.workspace,
        getEffectiveRegistryTag(localCacheTag, registryTagInput),
        cacheBackend,
        requestedPort,
    ...
    if (sccacheMode === 'proxy') {
      ...
      if (sccacheTag && sccacheStats && sccacheStats.compileRequests > 0) {
        ...
        if (sccacheStats.cacheHits === 0) {
          if (preflightKvHit) {
            core.warning(
              `sccache proxy saw 0 cache hits across ${sccacheStats.compileRequests} compile requests even though direct KV rows existed for '${sccacheTag}' before startup. Check sccache key churn, emitted tag semantics, and proxy read logs.`,
            );
            ...
            core.warning(
              `sccache proxy saw 0 cache hits across ${sccacheStats.compileRequests} compile requests and '${sccacheTag}' had a signed cache entry after shutdown, but direct KV rows were not visible. Check boringcache/one cli-version alignment and proxy KV publish logs.`,
            );
          } else {
            core.notice(
              `sccache proxy saw 0 cache hits across ${sccacheStats.compileRequests} compile requests and '${sccacheTag}' was not reported as direct KV rows during post-shutdown verification. This usually means a cold fill; check proxy publish logs if the next run also misses.`,
            );
          }
        }
      }
