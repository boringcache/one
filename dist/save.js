...
export async function run() {
    ...
    try {
        ...
        const saveEntries = parseEntries(genericEntries, 'save', {
            ...
            .map((entry) => `${entry.tag}:${entry.savePath}`);
        const portableArchiveArgs = supportsPortableArchiveArgs(cliVersion);
        if (enableCrossOsArchive) {
            assertCrossOsArchiveTransportSupported(cliVersion);
        }
        ...
        appendSaveExcludeArgs(args, excludes, cliVersion);
        if (inputs.failOnCacheError) {
            args.push('--fail-on-cache-error');
        }
        if (inputs.allowExternalSymlinks) {
            assertExternalSymlinkRoundTripSupported(cliVersion);
